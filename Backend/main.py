...
app.add_middleware(
    CORSMiddleware,
    # For local dev, allow any origin (token-based auth; no cookies).
    # This avoids "Failed to fetch" due to mismatched localhost/IPv6 origins.
    allow_origins=["*"],
...
async def query(request: QueryRequest):
    ...
    # Build context from evidence with timestamps
    context_parts = []
    for i, r in enumerate(results):
        meta = []
        if r.get("source_file"):
            meta.append(r["source_file"])
        if r.get("modality"):
            meta.append(r["modality"])
        
        # Add timestamp if available
        if r.get("timestamp_start") is not None:
            ts = f"Time: {r['timestamp_start']:.1f}s"
            if r.get("timestamp_end"):
                ts += f"-{r['timestamp_end']:.1f}s"
            meta.append(ts)
            
        meta_str = ", ".join(meta)
        text = r.get("text_content", "")[:500]
        context_parts.append(f"[{i+1}] ({meta_str}): {text}")
