...
def calc_extraction_confidence(chunks: list[dict]) -> float:
    ...
    for chunk in chunks:
        ...
        elif chunk.get("modality") == "image_description":
            # Vision LLM descriptions are high confidence
...
def calc_source_score(chunks: list[dict]) -> float:
    """More sources = higher confidence (relaxed for single-file tests)."""
    ...
    if unique_sources >= 5:
        ...
        return 0.8  # Was 0.4 - much friendlier for single-doc chat
    else:
        return 0.0
...
# Weight constants
WEIGHTS = {
    "embedding": 0.45,    # Boosted: If vector match is good, trust it
    "extraction": 0.20,
    "modality": 0.25,
    "sources": 0.10,      # Reduced: Don't penalize small number of sources
}
