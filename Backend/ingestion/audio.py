...
def _rechunk_by_sentences(segments: list[dict]) -> list[dict]:
    ...
    while preserving timing and confidence.
    """
    chunks = []
    current_text = ""
    current_start = None
    ...
    for seg in segments:
        ...
        if re.search(r'[.!?]\s*$', text):
            ...
            confidence = _calculate_confidence(avg_logprob, avg_no_speech)
    ...
    if current_text.strip():
        ...
        chunks.append({
            "text_content": current_text.strip(),
            "modality": "audio_transcript",
            "timestamp_start": current_start if current_start is not None else 0.0,
            "timestamp_end": current_end if current_end is not None else 0.0,
            "asr_confidence": confidence,
        })
...
async def parse_audio(file_path: Path) -> list[dict]:
    ...
    - Timestamps for alignment with video frames
    """
    try:
        ...
        result = model.transcribe(
            str(file_path),
            verbose=False,
            fp16=False, # Fix: UserWarning on CPU
        )
        ...
        # Re-chunk by sentences for semantic meaning while keeping timings
